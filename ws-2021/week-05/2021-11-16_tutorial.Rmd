...
This sheet makes use of two R packages that you may not yet have installed, `datasauRus` (which contains an educational dataset for demonstrating summary statistics) and `scico` (scientific colour map palettes). You can install them by running the code chunk below:

```{r, eval = F}
install.packages("scico") # Scientific colour map palettes
...
```
The summary statistics for each of the 13 datasets are the same. Run the code below to create a scatterplot for each of the 13 datasets.
...
netflix_titles %>% 
  ...
  ggplot(aes(year_added, n, colour = type)) +
  geom_line(size = 2, alpha = .75) +
  scico::scale_colour_scico_d(palette = "roma") +
  labs(
    x = "Year",
